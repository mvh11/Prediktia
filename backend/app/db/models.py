"""Modelos ORM — SQLAlchemy 2.x + Python 3.14 (Render/Neon).

Dentro de ``Mapped[]``: solo ``int``, ``str``, ``float``, ``datetime``, ``date`` o ``Any`` (JSONB).
Columnas nullable: ``Mapped[T]`` + ``nullable=True`` (sin Optional ni ``T | None``).
"""
...
class FixtureRow(Base):
    ...
    league_id: Mapped[int] = mapped_column(Integer, nullable=True, index=True)
    ...
    kickoff: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
...
class TeamRow(Base):
    ...
    external_id: Mapped[int] = mapped_column(Integer, unique=True, nullable=True, index=True)
...
class PredictionRow(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    acca_id: Mapped[str] = mapped_column(
        String(36),
...
class AccaHistoryRow(Base):
    ...
    roi: Mapped[float] = mapped_column(Float, nullable=True)
...
class ModelMetricRow(Base):
    ...
    scope: Mapped[str] = mapped_column(String(128), nullable=True)
